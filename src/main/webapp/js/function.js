...
function setEditor(path, holder){
	var oEditors = [];
...
function dateDiff(_date1, _date2) {
    ...
    */
    var date1Array = _date1.split("-");
    var date2Array = _date2.split("-"); 

    var date1Obj = new Date(date1Array[0], Number(date1Array[1])-1, date1Array[2]); 
    var date2Obj = new Date(date2Array[0], Number(date2Array[1])-1, date2Array[2]); 
      
    var betweenDay = (date2Obj.getTime() - date1Obj.getTime())/1000/60/60/24;  
 
    //return diff;
    return betweenDay;
}

// 앞날짜가 뒤날짜보다 작으면 true, 크거나 같으면 false
...
// 해당모선 스케쥴
function ship_schedule_pop(vsl, vyg, gb) {
	window.open('/schedule/ship_schedule.do?vsl='+vsl+'&vyg='+vyg+'&gb='+gb,'schedule_pop','width=1000, height=550, top=0, left=0,toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=yes, resizable=yes');
}